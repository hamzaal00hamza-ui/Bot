...
async def check_pending_api_orders(context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    يفحص الطلبات التي ما زالت بحالة معلقة عند Fastcard API ويرسل التأكيد
    للمستخدم حين تكتمل أو يسترجع المبلغ حين ترفض.

    يحل مشكلة: عندما يتجاوز الطلب مهلة الـ polling الأولية (45 ث) يبقى
    "pending/processing" بلا متابعة، فيصل المستخدم أبداً تأكيد.
    """
    ...
    for order in orders:
        ...
        if status not in _ACCEPTED and status not in _REJECTED:
            # لا تزال معلقة — لا نفعل شيئاً الآن
            continue
        ...
        if status in _ACCEPTED:
            ...
            try:
                ...
                logger.error(f"check_pending_api_orders: failed to notify user {user_id}: {e}")
            ...
            if config.ADMIN_ID:
                try:
                    ...
                    logger.error(f"admin notify failed for order #{order_id}: {e}")
            ...
            try:
                ...
                logger.error(f"check_pending_api_orders: refund failed for order #{order_id}: {e}")
            ...
            try:
                ...
                logger.error(f"check_pending_api_orders: failed to notify rejection to user {user_id}: {e}")
...
def schedule_jobs(app: Application) -> None:
    ...
    # ✅ الإصلاح: متابعة الطلبات المعلقة كل دقيقتين وإرسال التأكيد للمستخدمين
    job_queue.run_repeating(
        check_pending_api_orders,
        interval=120,  # كل 2 دقيقة
        first=30,      # ابدأ بعد 30 ثانية من تشغيل البوت
        name="check_pending_api_orders",
    )
    logger.info("✅ Pending API orders checker scheduled (every 2 min)")
