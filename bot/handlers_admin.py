...
(
    ...
    ADMIN_CHANNEL_INPUT,
    ADMIN_PRICE_INPUT,
    ADMIN_USDT_RATE_INPUT,
    ADMIN_USDT_CREDIT_UID,
...
async def cb_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin:status":
        await q.edit_message_text("🔌 جاري فحص حالة جميع الخدمات...")

        from . import fastcard, usdt as usdt_module
        from .shamcash import ShamCashError
        ...
        if not fastcard.is_enabled():
            lines.append("🏪 *Fastcard المتجر:* ❌ التوكن غير مضبوط")
        else:
            try:
                profile = await _aio.to_thread(fastcard.get_profile)
                name = profile.get("name") or profile.get("username") or "—"
                bal = profile.get("balance")
                bal_str = f" | رصيد: *{float(bal):,.2f}*".replace(",", "،") if bal is not None else ""
                lines.append(f"🏪 *Fastcard:* ✅ متصل | {name}{bal_str}")
            except Exception as e:
                lines.append(f"🏪 *Fastcard:* ❌ خطأ — `{e}`")

        # ===== سرياتيل كاش =====
        if not syriatel_cash.is_enabled():
            lines.append("📱 *سرياتيل كاش:* ❌ التوكن غير مضبوط")
        else:
            try:
                bal = await _aio.to_thread(syriatel_cash.get_balance, None, False)
                bal_str = f"{bal:,.0f} ل.س".replace(",", "،")
                lines.append(f"📱 *سرياتيل كاش:* ✅ متصل | رصيد: *{bal_str}*")
            except syriatel_cash.SyriatelCashError as e:
                lines.append(f"📱 *سرياتيل كاش:* ❌ {e.message}")
            except Exception as e:
                lines.append(f"📱 *سرياتيل كاش:* ❌ `{e}`")

        # ===== شام كاش =====
        from . import shamcash as _sc
        if not _sc.is_enabled():
            lines.append("💳 *شام كاش:* ❌ التوكن غير مضبوط")
        else:
            try:
                accs = await _aio.to_thread(_sc.list_accounts)
                active = next((a for a in accs if a.get("status") == "active"), None)
                if active:
                    acc_id = active.get("id", "—")
                    bals = await _aio.to_thread(_sc.get_balances, acc_id)
                    syp_bal = None
                    if isinstance(bals, list):
                        for b in bals:
                            if b.get("coin_id") == _sc.COIN_SYP:
                                syp_bal = b.get("balance")
                                break
                    elif isinstance(bals, dict):
                        syp_bal = bals.get("balance")
                    bal_str = f" | رصيد ل.س: *{float(syp_bal):,.0f}*".replace(",", "،") if syp_bal is not None else ""
                    lines.append(f"💳 *شام كاش:* ✅ متصل{bal_str}")
                else:
                    lines.append("💳 *شام كاش:* ⚠️ متصل لكن لا يوجد حساب active")
            except ShamCashError as e:
                lines.append(f"💳 *شام كاش:* ❌ {e.message}")
            except Exception as e:
                lines.append(f"💳 *شام كاش:* ❌ `{e}`")

        # ===== USDT / BSCScan =====
        if not usdt_module.is_enabled():
            lines.append("🪙 *USDT:* ❌ BSCSCAN_API_KEY غير مضبوط")
        else:
            try:
                bal = await _aio.to_thread(usdt_module.get_wallet_balance)
                rate = usdt_module.get_usdt_rate()
                wallet = usdt_module.get_wallet_address()
                short_wallet = wallet[:8] + "…" + wallet[-6:]
                if bal is not None:
                    lines.append(
                        f"🪙 *USDT:* ✅ متصل | رصيد: *{bal:.4f} USDT*\n"
                        f"   💱 السعر: {rate:,} ل.س | 📍 `{short_wallet}`".replace(",", "،")
                    )
                else:
                    lines.append(f"🪙 *USDT:* ⚠️ BSCScan متصل لكن تعذّر جلب الرصيد")
            except Exception as e:
                lines.append(f"🪙 *USDT:* ❌ `{e}`")

        lines.append("\n_آخر فحص: الآن_")
        text = "\n".join(lines)
        markup = kb.admin_status_panel()
        try:
...
async def cb_admin_usdt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج شاشة إدارة USDT — entry point منفصل ليتعامل مع admin:usdt:*"""
    q = update.callback_query
    await q.answer()
    if not is_admin(update):
        return ConversationHandler.END
    ...
    data = q.data
    from . import usdt as usdt_module

    # ===== الشاشة الرئيسية =====
    if data == "admin:usdt":
        stats = await asyncio.to_thread(db.get_usdt_stats)
        wallet = usdt_module.get_wallet_address()
        rate = usdt_module.get_usdt_rate()
        m = stats.get("matched", {})
        p = stats.get("pending", {})
        ...
        text = (
            ...
            "━━━━━━━━━━━━━━━━━\n\n"
            f"📍 المحفظة:\n`{wallet}`\n\n"
            f"💱 سعر USDT الحالي: *{rate:,} ل.س*\n".replace(",", "،") +
            "\n📊 *إحصائيات:*\n"
            f"  ✅ ناجحة:   {m.get('count',0)} إيداع — {m.get('usdt',0):.2f} USDT ≈ {m.get('syp',0):,.0f} ل.س\n".replace(",", "،") +
            f"  ⏳ معلّقة:  {p.get('count',0)} إيداع — {p.get('usdt',0):.2f} USDT\n"
            f"  ❌ منتهية:  {e.get('count',0)} إيداع"
        )
        await q.edit_message_text(text, reply_markup=kb.admin_usdt_panel(), parse_mode=ParseMode.MARKDOWN)
        return ConversationHandler.END

    # ===== آخر الإيداعات =====
    if data == "admin:usdt:deposits":
        rows = await asyncio.to_thread(db.get_usdt_recent_deposits, 20)
        if not rows:
            await q.answer("لا توجد إيداعات بعد.", show_alert=True)
            return ConversationHandler.END

        status_icons = {"matched": "✅", "pending": "⏳", "expired": "❌", "cancelled": "🚫"}
        lines = ["📋 *آخر 20 إيداع USDT*\n━━━━━━━━━━━━━━━━━"]
        for r in rows:
            icon = status_icons.get(r.get("status", ""), "❓")
            uname = r.get("username") or r.get("first_name") or str(r["user_id"])
            date = (r.get("created_at") or "")[:10]
            usdt_amt = float(r.get("amount_usdt") or 0)
            syp_amt = int(r.get("amount_syp") or 0)
            syp_str = f" ≈ {syp_amt:,} ل.س".replace(",", "،") if syp_amt else ""
            lines.append(f"{icon} @{uname} | {usdt_amt:.3f} USDT{syp_str} | {date}")
    ...
    # ===== الإيداعات المعلّقة فقط =====
    if data == "admin:usdt:pending":
        rows = await asyncio.to_thread(db.get_usdt_recent_deposits, 30, "pending")
        if not rows:
            await q.answer("لا توجد إيداعات معلّقة حالياً ✅", show_alert=True)
            return ConversationHandler.END

        lines = [f"⏳ *الإيداعات المعلّقة ({len(rows)})*\n━━━━━━━━━━━━━━━━━"]
        for r in rows:
            uname = r.get("username") or r.get("first_name") or str(r["user_id"])
            usdt_amt = float(r.get("amount_usdt") or 0)
            expires = (r.get("expires_at") or "")[:16]
            uid = r["user_id"]
            lines.append(f"⏳ @{uname} (`{uid}`) | {usdt_amt:.4f} USDT | ينتهي: {expires}")
    ...
    # ===== تعديل سعر USDT =====
    if data == "admin:usdt:set_rate":
        cur_rate = usdt_module.get_usdt_rate()
        await q.edit_message_text(
            "✏️ *تعديل سعر USDT*\n"
            "━━━━━━━━━━━━━━━━━\n\n"
            f"السعر الحالي: *{cur_rate:,} ل.س / USDT*\n\n".replace(",", "،") +
            "📝 أرسل السعر الجديد (مثال: `13500`).\n\n"
            "_هذا السعر يُستخدم لتحويل إيداعات USDT إلى رصيد بالليرة السورية._",
            reply_markup=kb.admin_usdt_cancel(),
            parse_mode=ParseMode.MARKDOWN,
        )
        return ADMIN_USDT_RATE_INPUT

    # ===== إضافة رصيد USDT يدوياً =====
    if data == "admin:usdt:manual_credit":
        await q.edit_message_text(
            "💰 *إضافة رصيد USDT يدوياً*\n"
            "━━━━━━━━━━━━━━━━━\n\n"
            "📝 أرسل *آيدي المستخدم* الذي تريد إضافة الرصيد له:",
            reply_markup=kb.admin_usdt_cancel(),
            parse_mode=ParseMode.MARKDOWN,
        )
        return ADMIN_USDT_CREDIT_UID

    return ConversationHandler.END


async def msg_admin_usdt_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يستقبل سعر USDT الجديد ويحفظه."""
    from . import usdt as usdt_module
    txt = (update.message.text or "").strip().replace(",", "").replace("،", "").replace(" ", "")
    txt = txt.translate(str.maketrans("٠١٢٣٤٥٦٧٨٩", "0123456789"))
    try:
        new_rate = int(txt)
        if not (1000 <= new_rate <= 10_000_000):
            raise ValueError
    except (ValueError, TypeError):
        await update.message.reply_text(
            "⚠️ أدخل رقم صحيح بين 1,000 و 10,000,000.".replace(",", "،"),
            reply_markup=kb.admin_usdt_cancel(),
        )
        return ADMIN_USDT_RATE_INPUT

    usdt_module.set_usdt_rate(new_rate)
    await update.message.reply_text(
        f"✅ *تم تحديث سعر USDT إلى {new_rate:,} ل.س*".replace(",", "،"),
        reply_markup=kb.admin_usdt_panel(),
        parse_mode=ParseMode.MARKDOWN,
    )
    return ConversationHandler.END


async def msg_admin_usdt_credit_uid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يستقبل آيدي المستخدم للإضافة اليدوية."""
    txt = (update.message.text or "").strip()
    txt = txt.translate(str.maketrans("٠١٢٣٤٥٦٧٨٩", "0123456789"))
    try:
        uid = int(txt)
    ...
    if not user:
        await update.message.reply_text(
            f"⚠️ لم أجد مستخدماً بالآيدي `{uid}`.",
            reply_markup=kb.admin_usdt_cancel(),
            parse_mode=ParseMode.MARKDOWN,
        )
        return ADMIN_USDT_CREDIT_UID

    context.user_data["usdt_credit_uid"] = uid
    uname = user.get("username") or user.get("first_name") or str(uid)
    balance = float(user.get("balance") or 0)
    await update.message.reply_text(
        f"✅ المستخدم: @{uname} (`{uid}`)\n"
        f"💼 رصيده الحالي: *{balance:,.0f} ل.س*\n\n".replace(",", "،") +
        "📝 أرسل المبلغ *بالـ USDT* الذي تريد إضافته (مثال: 5.5):",
        reply_markup=kb.admin_usdt_cancel(),
        parse_mode=ParseMode.MARKDOWN,
    )
    return ADMIN_USDT_CREDIT_AMOUNT


async def msg_admin_usdt_credit_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """يستقبل مبلغ USDT ويضيف الرصيد بالليرة يدوياً."""
    from . import usdt as usdt_module
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ أدخل مبلغ USDT صحيح (مثال: 5 أو 10.5)",
            reply_markup=kb.admin_usdt_cancel(),
        )
        return ADMIN_USDT_CREDIT_AMOUNT

    uid = context.user_data.get("usdt_credit_uid")
    if not uid:
        await update.message.reply_text("⚠️ انتهت الجلسة، ابدأ من جديد.", reply_markup=kb.back_to_admin())
        return ConversationHandler.END

    rate = usdt_module.get_usdt_rate()
    amount_syp = int(amount_usdt * rate)

    state = await asyncio.to_thread(db.update_balance, uid, float(amount_syp), True)
    if not state:
        await update.message.reply_text("❌ فشل تحديث الرصيد.", reply_markup=kb.admin_usdt_panel())
        return ConversationHandler.END

    new_balance = float(state.get("balance") or 0)
    user = await asyncio.to_thread(db.get_user, uid)
    uname = (user or {}).get("username") or (user or {}).get("first_name") or str(uid)

    await update.message.reply_text(
        "✅ *تم إضافة الرصيد يدوياً!*\n"
        "━━━━━━━━━━━━━━━━━\n\n"
        f"👤 المستخدم: @{uname} (`{uid}`)\n"
        f"🪙 المبلغ: *{amount_usdt:.4f} USDT*\n"
        f"💰 المضاف: *{amount_syp:,} ل.س*\n".replace(",", "،") +
        f"💼 الرصيد الجديد: *{new_balance:,.0f} ل.س*".replace(",", "،"),
        reply_markup=kb.admin_usdt_panel(),
        parse_mode=ParseMode.MARKDOWN,
    )
    ...
    try:
        await context.bot.send_message(
            chat_id=uid,
            text=(
                "✅ *تم إضافة رصيد USDT يدوياً!*\n\n"
                f"🪙 {amount_usdt:.4f} USDT\n"
                f"💰 *{amount_syp:,} ل.س*\n".replace(",", "،") +
                f"💼 رصيدك الآن: *{new_balance:,.0f} ل.س*".replace(",", "،")
            ),
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception:
    ...
    context.user_data.pop("usdt_credit_uid", None)
    return ConversationHandler.END
...
def register_admin_handlers(app):
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            ...
            ADMIN_USDT_RATE_INPUT: [
                CallbackQueryHandler(cb_admin_usdt, pattern=r"^admin:usdt"),
                back_handler,
                MessageHandler(filters.TEXT & ~filters.COMMAND, msg_admin_usdt_rate),
            ],
            ADMIN_USDT_CREDIT_UID: [
                CallbackQueryHandler(cb_admin_usdt, pattern=r"^admin:usdt"),
                back_handler,
                MessageHandler(filters.TEXT & ~filters.COMMAND, msg_admin_usdt_credit_uid),
            ],
            ADMIN_USDT_CREDIT_AMOUNT: [
                CallbackQueryHandler(cb_admin_usdt, pattern=r"^admin:usdt"),
                back_handler,
                MessageHandler(filters.TEXT & ~filters.COMMAND, msg_admin_usdt_credit_amount),
            ],
        },
    ...
    usdt_admin_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_admin_usdt, pattern=r"^admin:usdt")],
        states={
            ADMIN_USDT_RATE_INPUT: [
                CallbackQueryHandler(cb_admin_usdt, pattern=r"^admin:usdt"),
                back_handler,
    ...
    app.add_handler(usdt_admin_conv)
